...
export function AddToCart({ initialCartCount }: { initialCartCount: number }) {
  ...
  const [, setOptimisticCartCount] = useCart();
  ...
  return (
    <button
      className="relative w-full items-center space-x-2 rounded-lg bg-blue-500 px-3 py-1  text-sm font-medium text-white hover:bg-blue-500/90 disabled:text-white/70"
      onClick={addToCart}
      disabled={isPending}
    >
      Add to Cart
